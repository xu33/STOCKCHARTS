...
class TimeTrendChart extends EventEmitter {
  ...
  initTouchEvents() {
    ...
    const drawTip = (data, datum) => {
      ...
      tips.each(function(d, i) {
        ...
        if (i === 0) {
          ...
          if (outerX > width - bbox.width - TIP_PADDING * 2)
            outerX = width - bbox.width - TIP_PADDING * 2;
        } else if (i === 1) {
          // 左侧
          outerX = d.x + 1; // 加1防止贴边
          ...
          if (outerY > baseHeight - bbox.height / 2)
            outerY = baseHeight - bbox.height / 2;
        }
    ...
    const handleTouch = function() {
      ...
      if (that.showVolume) {
        // 更新成交量文字
        that.volumeLeftAxisElement
          .select('.tick text')
          .text(`成交量:${formatMoney(datum.volume)}`);
      }
  ...
  createVerticalAxis() {
    ...
    var helpScale = d3
      .scaleOrdinal()
      .domain([0, 1, 2, 3, 4])
      .range([0, step * 1, step * 2, step * 3, this.baseHeight]);
    var rightAxis = d3
      .axisLeft(helpScale)
      .tickSize(this.width)
      .tickFormat('')
      .ticks(4);
    var rightAxisElement = svg
      .append('g')
      .attr('class', 'chart-axis')
  ...
  renderVolumes() {
    ...
    this.volumeWrapper.attr(
      'transform',
      `translate(0, ${this.baseHeight + BOTTOM_MARGIN})`
    );

    // 绘制量图数轴
    // 保存方便后续更新
    this.volumeLeftAxisElement = this.volumeWrapper
      .append('g')
      .attr('class', 'chart-axis');
    ...
    // 量图垂直辅助线
    var axisBottom = d3
      .axisTop(this.staticScale)
      .tickSize(this.volHeight - 21)
      .tickFormat('');
    var axisBottomElement = this.volumeWrapper
      .append('g')
    ...
    var axisTop = d3
      .axisBottom(this.staticScale)
      .tickSize(0)
      .tickFormat('');
    var axisTopElement = this.volumeWrapper
      .append('g')
      .attr('class', 'chart-axis')
      .call(axisTop);
    this.updateVolumes();
    ...
    var scale = d3
      .scaleOrdinal()
      .domain([1, 2, 3])
      .range([0, 20, this.volHeight]);
    var axisLeft = d3.axisLeft(scale).tickSize(this.width);
    var axisLeftElement = this.volumeWrapper
      .append('g')
      .attr('class', 'chart-axis')
      .attr('transform', `translate(${this.width - 1}, 0)`)
      .call(axisLeft);
  ...
  updateVolumesYAxis() {
    ...
    var max = d3.max(this.data, d => d.volume);
    var curr =
      this.data.length > 0 ? this.data[this.data.length - 1].volume : 0;

    var scale = d3
      .scaleOrdinal()
      .domain([`成交量:${formatMoney(curr)}`, formatMoney(max), ''])
      .range([0, 20, this.volHeight]);

    var axis = d3.axisRight(scale).tickSize(0);
    this.volumeLeftAxisElement.call(axis);

    this.volumeLeftAxisElement
      .selectAll('.tick text')
