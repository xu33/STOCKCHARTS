...
class StockChart extends EventEmitter {
  ...
  constructor(selector, options) {
    ...
    this.candleGroup = this.svg.append('g').attr('class', 'candles');

    // 十字线交互
    this.crosshair = new Crosshair(this.svg, options.width, options.height);
    this.crosshair.createEventOverlay();
  ...
  renderOverviewArea() {
    ...
    var container = this.element
      .append('svg')
      .attr('height', this.overviewAreaHeight + 1)
    ...
    var scaleY = d3
      .scaleLinear()
      .domain([min, max])
      .range([this.overviewAreaHeight, 10]);
    var scaleX = d3
      .scaleLinear()
    ...
    var axisTop = d3
      .axisBottom(scaleForAxis)
      .tickSize(this.overviewAreaHeight)
      .tickFormat('');
    var axisBottomElement = container
      .append('g')
      .attr('class', 'axis')
      .attr('transform', `translate(0, ${this.overviewAreaHeight})`)
      .call(axisBottom);
  ...
  initBrush() {
    ...
    var brushed = function(d, i) {
      ...
      var offset = Math.round(newDomain[0]);
      var end = Math.round(newDomain[1]) + 1;

      var dataAtCurrentBrushRange = candleData.slice(offset, end);
      // 设置当前需要画的部分
      self.options.candleData = dataAtCurrentBrushRange;
  ...
  candleSticks() {
    ...
    let scaleY = d3
      .scaleLinear()
      .domain([min, max])
      .range([this.candleStickAreaHeight - margin, 0 + margin]);

    this.scaleBandX = scaleX;

    let group = this.candleGroup;
    let candleSelection = group.selectAll('.bar').data(candleData, d => d.date);
  ...
  paintShadowLines(group, scaleX, scaleY) {
    ...
    let selection = group.selectAll('.shadow').data(candleData);
  ...
  paintMacdArea() {
    ...
    var group = this.element
      .append('svg')
      .attr('width', width + 1)
      .attr('height', this.macdAreaHeight + 1)
      .attr('class', 'macd-container');
    ...
    group
      .append('rect')
      .attr('width', width)
      .attr('height', this.macdAreaHeight)
      .attr('stroke', '#ccc')
      .attr('fill', 'none');
  ...
  updateMacdArea() {
    ...
    var scaleX = d3
      .scalePoint()
      .domain(candleData.map((item, index) => index))
      .range([0, width])
      .padding(0.2);
  ...
  volumes() {
    ...
    if (!this.volumeGroup) {
      this.volumeGroup = svg.append('g').attr(
        'transform',
        `translate(
        0, 
        ${height + MARGIN_BOTTOM - 1}
      )`
      );

      this.volumeScaleY = d3.scaleLinear().range([VOL_HEIGHT, 10]);
      ...
      // this.volumeGroup
      //   .append('path')
      //   .datum([{ x: 0, y: VOL_HEIGHT / 2 }, { x: width, y: VOL_HEIGHT / 2 }])
      //   .attr('d', line)
      //   .attr('stroke', '#ccc');
    ...
    let min = d3.min(candleData, d => d.volume);
    let max = d3.max(candleData, d => d.volume);

    this.volumeScaleY.domain([min, max]);

    let volBars = this.volumeGroup.selectAll('.bar').data(candleData);
    volBars.exit().remove();
    ...
    volBars
      ...
      .attr('height', d => VOL_HEIGHT - this.volumeScaleY(d.volume))
      .attr('fill', d => {
        if (d.open < d.close) {
          return WIN_COLOR;
        } else {
  ...
  events() {
    ...
    this.svg
      .on('mouseenter', function() {
        ...
        self.currentScaleY = d3
          .scaleLinear()
          .domain([min, max])
          .range([self.candleStickAreaHeight, 0]);
